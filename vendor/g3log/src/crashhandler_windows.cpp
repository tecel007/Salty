...
//#endif
   ...
   LONG WINAPI exceptionHandling(EXCEPTION_POINTERS *info, const std::string &handler) {
      ...
      char buff[256] = "";  sprintf(buff, "GTA5.exe+0x%I64X", (uint64_t)info->ExceptionRecord->ExceptionAddress - (uint64_t)GetModuleHandle(NULL));

      std::ostringstream fatal_stream;
      fatal_stream << std::endl << ",CODE," << g3::internal::exitReasonName(g3::internal::FATAL_EXCEPTION, exception_code);
      //fatal_stream << std::endl << ",PID," << getpid();
      fatal_stream << std::endl << ",RIP," << buff;
