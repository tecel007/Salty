...
#include "sync_tree_message_buffer.hpp"
#include "sync_tree_server_game_state.hpp"
...
namespace fx::sync
{
	...
	{
		if ((std::get<0>(ids) & state.sync_type) == 0)
		{
			return false;
		}

		if (std::get<2>(ids) && !(state.object_flags & std::get<2>(ids)))
		{
			return false;
		}

		if ((std::get<1>(ids) & state.sync_type) != 0)
		{
	...
	struct ParentNode : public NodeBase
	{
		...
		{
			char buf[256];
			sprintf(buf, ",ParentNode,OK,%d", state.buffer.pos);
			big::misc::log_helper(buf, true);
	...
	struct NodeWrapper : public NodeBase
	{
		...
		virtual bool Parse(SyncParseState& state) final override
		{
			...
			{
				char buf[256];
				sprintf(buf, ",NodeWrapper,OK,%d", state.buffer.pos);
				big::misc::log_helper(buf, true);
	...
	struct CSectorPositionDataNode
	{
		...
		bool Parse(SyncParseState& state)
		{
			posX = state.buffer.ReadFloat(12, 54.0f);
			posY = state.buffer.ReadFloat(12, 54.0f);
			posZ = state.buffer.ReadFloat(12, 69.0f);
	...
	struct CEntityOrientationDataNode
	{
		...
		bool Parse(SyncParseState& state)
		{
			auto largest = state.buffer.Read<uint32_t>(2);
			auto integer_a = state.buffer.Read<uint32_t>(11);
			auto integer_b = state.buffer.Read<uint32_t>(11);
			auto integer_c = state.buffer.Read<uint32_t>(11);
	...
	struct CPedHealthDataNode
	{
		...
		bool Parse(SyncParseState& state)
		{
			...
			causeOfDeath = state.buffer.Read<int>(32);
	...
	struct SyncTree : public SyncTreeBase
	{
		...
		virtual bool GetModelHash(uint32_t* modelHash) override
		{
			...
			if (hasPcn)
			{
				*modelHash = pedCreationNode->model;
				return true;
			}
