...
namespace memory
{
	...
	{
		return h.as<uintptr_t>() >= begin().as<uintptr_t>() && h.as<uintptr_t>() <= end().as<uintptr_t>();
	}

	static bool pattern_matches(uint8_t* target, const std::optional<uint8_t>* sig, std::size_t length)
	{
	...
	handle range::scan(pattern const &sig)
	{
		...
		for (uintptr_t i = 0; i < m_size - length; ++i)
		{
